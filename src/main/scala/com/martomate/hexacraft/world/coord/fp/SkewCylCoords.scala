...
class SkewCylCoords private (_x: Double, _y: Double, _z: Double, fixZ: Boolean)(implicit val cylSize: CylinderSize)
  ...
  def toBlockCoords: BlockCoords = BlockCoords(x / CylinderSize.y60, y / 0.5, z / CylinderSize.y60, fixZ)

  override def offset(dx: Double, dy: Double, dz: Double): SkewCylCoords = SkewCylCoords(x + dx, y + dy, z + dz, fixZ)
}
