...
//    BlockCoords(0, (bodyLength + legLength) / 32f + headDepth / 2 / 32f, 0).toCylCoords,
    new BlockCoords(0, (bodyLength + legLength) / 32f + headRadius / 32f * CylinderSize.y60, 0, false).toCylCoords,
  ...
  private val leftBodyhalf = new BasicEntityPart(
    ...
    new BlockCoords(0, legLength / 32f, -0.5f * bodyRadius / 32, false).toCylCoords,
    new Vector3f(0, 0, 0),
  ...
  private val rightArm = new BasicEntityPart(
    ...
    new BlockCoords(0, (legLength + bodyLength - armRadius * CylinderSize.y60.toFloat) / 32f, 0.5f * 2 * bodyRadius / 32 + 0.5f * armRadius / 32, false).toCylCoords,
  ...
  private val rightLeg = new BasicEntityPart(
    ...
    new BlockCoords(0, legLength / 32f, 0.5f * legRadius / 32 + 0.001f, false).toCylCoords,
