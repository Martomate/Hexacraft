...
class WindowSystem(glfw: GlfwWrapper) {
  ...
  def primaryMonitor: Monitor = new Monitor(Monitor.Id(glfw.glfwGetPrimaryMonitor()), glfw)
  ...
  def monitors: Seq[Monitor] =
    ...
    do res += new Monitor(Monitor.Id(monitors.get), glfw)
  ...
  def createWindow(settings: WindowSettings): Option[Window] =
    ...
    if id != 0 then Some(new Window(Window.Id(id), glfw)) else None
}
