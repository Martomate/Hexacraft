...
class NoiseInterpolator3D(xSize: Int, ySize: Int, zSize: Int, sampler: (Int, Int, Int) => Double) {
  private val noiseSamples = for (i <- 0 to xSize; j <- 0 to ySize; k <- 0 to zSize) yield sampler(i, j, k)
  private val stride2 = (ySize + 1) * (zSize + 1)
  private val stride1 =  zSize + 1
  ...
  def apply(x: Int, y: Int, z: Int): Double = interpolate(x / xSize, y / ySize, z / zSize, 
      (x % xSize) / xSize.toDouble, (y % ySize) / ySize.toDouble, (z % zSize) / zSize.toDouble)
}
