...
class Chunk(val coords: ChunkRelWorld, val world: World) {
  ...
  val neighbors: Array[Option[Chunk]] = Array.tabulate(8)(i => {
    ...
    val c2 = ChunkRelWorld(coords.X + dx, coords.Y + dy, coords.Z + dz, world.size)
...

  {
    ...
    if (file.isFile) {
      ...
      for (i <- 0 until 16; j <- 0 until 16; k <- 0 until 16) {
        ...
        if (noise > limit) storage.setBlock(new BlockState(BlockRelChunk(i, j, k, world.size).withChunk(coords), world,
  ...
  private def onBlockModified(coords: BlockRelChunk): Unit = {
    ...
    for (i <- 0 until 8) {
      ...
      val c2 = BlockRelChunk((coords.cx + off._1 + 16) % 16, (coords.cy + off._2 + 16) % 16, (coords.cz + off._3 + 16) % 16, world.size)
