...
class NBTUtilTest extends FlatSpec with Matchers {
  ...
  val byteArr = mutable.WrappedArray.make[Byte](Seq(42, 13, 42, -42).map(_.toByte).toArray)
  val byteArrCTag = makeCTag(new ByteArrayTag("tag", byteArr.array))
  val shortArr = mutable.WrappedArray.make[Short](Seq(4242, 1313, 4242, -4242).map(_.toShort).toArray)
  val shortArrCTag = makeCTag(new ShortArrayTag("tag", shortArr.array))
  ...
  testGetOption("getByteArray", NBTUtil.getByteArray, byteArrCTag, byteArr)
  testGetOption("getShortArray", NBTUtil.getShortArray, shortArrCTag, shortArr)
