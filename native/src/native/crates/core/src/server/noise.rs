...
mod tests_3d {
    ...
    fn fixed_for_same_input() {
        ...
        .map(|(x, z)| (x as f64 * 0.321, z as f64 * 0.321));

        let expected_noise: [_; 4] = [
            -0.007732983530836253,
            -0.00806369532523725,
            -0.01095986822578543,
            -0.011290221741594449,
        ];

        // These assertions act as regression tests
        assert_eq!(
            coords.map(|(x, z)| _gen.gen_wrapped_noise(x, z, 1.234)),
            expected_noise
        );
    }
...
mod tests_4d {
    ...
    fn fixed_for_same_input() {
        ...
        .map(|(x, y, z)| (x as f64 * 0.321, y as f64 * 0.321, z as f64 * 0.321));

        let expected_noise: [_; 8] = [
            -0.021290646695559346,
            -0.021698750769715336,
            -0.014903425882898346,
            -0.015316154349014104,
            -0.018046947291153404,
            -0.01845108392270608,
            -0.011659701177194668,
            -0.012068462455492309,
        ];

        // These assertions act as regression tests
        assert_eq!(
            coords.map(|(x, y, z)| _gen.gen_wrapped_noise(x, y, z, 1.234)),
            expected_noise
        );
    }
