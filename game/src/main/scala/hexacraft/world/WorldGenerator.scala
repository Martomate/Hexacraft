...
class WorldGenerator(worldGenSettings: WorldGenSettings)(using cylSize: CylinderSize) {
  ...
  private def sampleNoise3D(noise: NoiseGenerator3D): (Int, Int) => Double = {
    val radius = cylSize.radius
    (x, z) => {
      val c = BlockCoords(x, 0, z).toCylCoords
      noise.genWrappedNoise(x, z, radius)
    }
  }
...
object WorldGenerator {
  def makeSampler2D(coords: ColumnRelWorld, noise: (Int, Int) => Double): Data2D = {
    ...
    Data2D.interpolate(4, 4, Data2D.evaluate(samplingPoints, noise))
  }

  def makeSampler3D(coords: ChunkRelWorld, noise: (Int, Int, Int) => Double): Data3D = {
    ...
    Data3D.interpolate(4, 4, 4, Data3D.evaluate(samplingPoints, noise))
  }
  ...
  object Pos {
    def eval(bc: BlockCoords)(f: Pos => Double)(using cylSize: CylinderSize): Double = {
      val c = bc.toCylCoords
      f(Pos(c.x, c.y, c.z, cylSize.radius))
    }

    def eval2(f: Pos => Double)(using cylSize: CylinderSize): (Int, Int) => Double = { (x, z) =>
