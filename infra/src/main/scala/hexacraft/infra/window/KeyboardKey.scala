...
object KeyboardKey {
  ...
  object Letter {
    ...
    def unapply(key: KeyboardKey): Option[Char] = {
      if key >= GLFW.GLFW_KEY_A && key <= GLFW.GLFW_KEY_Z then {
        Some(((key - GLFW.GLFW_KEY_A) + 'A'.toInt).toChar)
      } else {
        None
      }
    }
  ...
  object Digit {
    ...
    def unapply(key: KeyboardKey): Option[Byte] = {
      if key >= GLFW.GLFW_KEY_0 && key <= GLFW.GLFW_KEY_9 then {
        Some((key - GLFW.GLFW_KEY_0).toByte)
      } else {
        None
      }
    }
  ...
  object Function {
    ...
    def unapply(key: KeyboardKey): Option[Byte] = {
      if key >= GLFW.GLFW_KEY_F1 && key <= GLFW.GLFW_KEY_F25 then {
        Some(((key - GLFW.GLFW_KEY_F1) + 1).toByte)
      } else {
        None
      }
    }
